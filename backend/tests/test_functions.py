...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(
                "logs", f'test_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
            )
        ),
...
def validate_trade_execution(order, side, size):
    ...
    assert order["status"] in [
        "open",
        "closed",
    ], f"{side.capitalize()} order should be open or closed"
    assert order["side"] == side, f"Order side should be {side}"

    # Use a small tolerance for float comparison
    tolerance = 1e-8
    assert (
        abs(float(order["amount"]) - size) < tolerance
    ), f"Order amount {order['amount']} should be close to {size}"
...
def test_trade_execution(bot):
    ...
    try:
        ...
        size = calculate_position_size(
            equity, bot.cfg.RISK_PER_TRADE, current_price, sl_price
        )
        ...
        if size < MIN_ORDER_SIZE:
            logger.warning(
                f"Position size {size} is below minimum {MIN_ORDER_SIZE}, adjusting..."
            )
            size = MIN_ORDER_SIZE
...
def test_indicators(bot):
    ...
    try:
        ...
        assert len(ema_fast) == len(
            closes
        ), "EMA Fast should have same length as closes"
        assert len(ema_slow) == len(
            closes
        ), "EMA Slow should have same length as closes"
