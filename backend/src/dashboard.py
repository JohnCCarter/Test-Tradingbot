...
def setup_logging():
    ...
    general_log_file = os.path.join(logs_dir, 'dashboard.log')
    ...
    bot_log_file = os.path.join(logs_dir, 'bot_errors.log')
    ...
    for handler in [file_handler, bot_file_handler]:
        handler.addFilter(lambda record: not record.getMessage().startswith('Request URL:'))
        handler.addFilter(lambda record: not record.getMessage().startswith('Request Method:'))
        handler.addFilter(lambda record: not record.getMessage().startswith('Request Headers:'))
        handler.addFilter(lambda record: not record.getMessage().startswith('Request JSON:'))
        handler.addFilter(lambda record: not record.getMessage().startswith('Request Form Data:'))
        handler.addFilter(lambda record: not record.getMessage().startswith('Request Args:'))
...
def get_current_price():
    try:
        ...
        alerts = []
        for alert in trading_state['price_alerts']:
            if alert['type'] == 'above' and current_price > alert['price']:
                alerts.append(f"Pris över {alert['price']}")
            elif alert['type'] == 'below' and current_price < alert['price']:
                alerts.append(f"Pris under {alert['price']}")
